...
vector<long> KGraph::FindDegeneracyOrdering()
{
	...
	//pos.resize(n);	// pos[v] is position of vertex v in vert
	vector<long> pos(n);

	//degeneracyordering.resize(n);	// vert[v] is the v-th vertex in the ordering
	vector<long> degeneracyordering(n);

	for (long i = 0; i < n; i++)
	{
		pos[i] = bin[rd[i]];
		degeneracyordering[pos[i]] = i;
		bin[rd[i]]++;
	}
	...
	for (long i = 0; i < n; i++)
	{
		long minv = degeneracyordering[i];	// this is a min-degree vertex in the remaining graph
		bin[rd[minv]]++;
		degeneracy = max(degeneracy, rd[minv]);
		...
		for (long j = 0; j < degree[minv]; j++) // adjust the degrees of the neighbors of v
		{
			...
			{
				//rightadj[minv].push_back(u);
				if (rd[u] == rd[minv])
				{
					long pw = bin[rd[minv]];	// the first position of the bin that contains vertex minv
					long w = degeneracyordering[pw];					// the vertex in that position
					if (u != w)						// if u is not the first vertex in the bin, swap u and w
					{
						degeneracyordering[pw] = u;
						degeneracyordering[pos[u]] = w;
						pos[w] = pos[u];
						pos[u] = pw;
					}
					bin[rd[minv] - 1] = pos[minv] + 1;
					bin[rd[u]]++;
					rd[u]--;
				}
				else
